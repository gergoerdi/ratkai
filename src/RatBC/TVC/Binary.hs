...
module RatBC.TVC.Binary (fixupSkips, assemble, writeFiles) where
...
assemble game@Game{..} = game
    ...
    , msgs2 = Const . mconcat . msgs {-(mconcat . map (BL.singleton . fromIntegral . ord)) -} . elems . runIdentity $ msgs2
...
assertNoSkips = id -- map assertNoSkip
